 ...
 *
 * This file comes from WiiFlow with endianness adjustments and slight changes.
 *
 *******************************************************************************/
#include <stdlib.h>
#include <unistd.h>
...
// Return the bytes needed to store a texture (taken from libogc)
...
static inline void _convertToRGB565(u8 *dst, const u8 *src, u32 width, u32 height)
{
	...
	for (u32 y = 0; y < height; ++y)
		...
		{
			u32 i = (x + y * width) * 4;
			dst16[coordsRGB565(x, y, width)] = __builtin_bswap16( ((src[i] & 0xF8) << 8) | ((src[i + 1] & 0xFC) << 3) | (src[i + 2] >> 3) );
		}
}
...
TexErr STexture::fromPNG(TexData &dest, const u8 *buffer, u8 f, u32 minMipSize, u32 maxMipSize, bool reduce_alpha)
{
	...
	if (maxLODTmp > 0)
	{
		...
		int error = 0;
		error = PNGU_DecodeToRGBA8(ctx, imgProp.imgWidth, imgProp.imgHeight, tmpData2, 0, 0xFF);
		if(error)
		{
			sprintf(buferror, "PNGU_DecodeToRGBA8 Error %d\n", error);
			return TE_ERROR;
		}
	...
	else
	{
		...
		if(dest.data == NULL)
		{
			printf("maxLODTmp <= 0 dest.data Null\n");
